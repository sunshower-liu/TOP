...
def main():
    ...
    # #modeldir = f"re_n{args.noise_level}_" + modeldir
    # modeldir = f"re_{args.fact_file[5:10]}_" + modeldir
    # modeldir = f'r{args.replace}_{args.model_name.replace("/", "_")}'
    # modeldir = f"n{args.noise_level}_" + modeldir
    # output_dir = args.output_dir.format(model_name=modeldir)
    output_dir = args.output_dir.format(model_name="top_re_causal_"+args.model_name.replace("/", "_"))
    output_dir2 = args.output_dir.format(model_name="top_re_causal_"+args.model_name.replace("/", "_") + "_edited")
    result_dir = f"{output_dir}/cases"
    result_dir2 = f"{output_dir2}/cases"
    ...
    if isinstance(noise_level, str):
        if noise_level.startswith("s"):
            ...
            noise_level = factor * collect_embedding_std(
                mt, [k["requested_rewrite"]["subject"] for k in knowns]
            )
    ...
    for knowledge in tqdm(knowns):
        ...
        subject = knowledge["requested_rewrite"]["subject"]
        prompt = knowledge["requested_rewrite"]["prompt"].format(subject)
        attribute_true = knowledge["requested_rewrite"]["target_true"]["str"]
        attribute_new = knowledge["requested_rewrite"]["target_new"]["str"]
        
        mt2 = ModelAndTokenizer(f"gpt2-top-{known_id}", torch_dtype=torch_dtype)
        
        for kind in ["mlp"]:
            kind_suffix = f"_{kind}" if kind else ""
            filename = f"{result_dir}/knowledge_{known_id}{kind_suffix}.npz"
            filename2 = f"{result_dir2}/knowledge_{known_id}{kind_suffix}.npz"
            if not os.path.isfile(filename):
                result = calculate_hidden_flow_re(
                    mt,
                    prompt,
                    subject,
                    expect=attribute_true,
                    kind=kind,
                    noise=noise_level,
            ...
            if not os.path.isfile(filename2):
                result2 = calculate_hidden_flow_re(
                    mt2,
                    prompt,
                    subject,
                    expect=attribute_new,
                    kind=kind,
                    noise=noise_level,
                    uniform_noise=uniform_noise,
                    replace=args.replace,
    ...
    def patch_rep(x, layer):
        if layer == embed_layername:
            ...
            if tokens_to_mix is not None:
                if type(tokens_to_mix)==list: 
                    b0, e0 = tokens_to_mix[0]
                    b, e = tokens_to_mix[1]
                    x[1:, b0:e0] += noise_fn(
                        torch.from_numpy(prng(x.shape[0] - 1, e0 - b0, x.shape[2]))
                    ).to(x.device)
                    x[1:, b:e] += noise_fn(
...
def calculate_hidden_flow_re(
    ...
    if expect is not None and answer.strip() != expect:
        return dict(correct_prediction=False)
    e_range = find_token_range_re(mt.tokenizer, inp["input_ids"][0], subject)
    '''
    if token_range == "subject_last":
        token_range = [e_range[1] - 1]
    elif token_range is not None:
...
def plot_trace_heatmap_re(result, savepdf=None, title=None, xlabel=None, modelname=None):
    differences = result["scores"]
    low_score = result["low_score"]
    answer = result["answer"]
    kind = (
    ...
    window = result.get("window", 10)
    labels = list(result["input_tokens"])
    if type(result["subject_range"]) == list:
        for i in range(*result["subject_range"][0]):
            labels[i] = labels[i] + "*"
        for i in range(*result["subject_range"][1]):
            labels[i] = labels[i] + "*"
    else:
        for i in range(*result["subject_range"]):
            labels[i] = labels[i] + "*"
...
def find_token_range_re(tokenizer, token_array, substring):
    toks = decode_tokens(tokenizer, token_array)
    whole_string = "".join(toks)
    char_loc = whole_string.index(substring)
    loc = 0
    tok_start_e = None
    tok_start, tok_end = None, len(toks)
    for i, t in enumerate(toks):
        loc += len(t)
        if tok_start_e is None and loc > char_loc:
            tok_start_e = i
        if tok_start is None and loc > char_loc + len(substring):
            tok_start = i
            break
    return  [(0,tok_start_e),(tok_start,tok_end)] if tok_start_e > 0 else (tok_start, tok_end)
