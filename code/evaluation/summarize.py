...
def main(
    ...
    for run_dir in (RESULTS_DIR / dir_name if not abs_path else dir_name).iterdir():
        ...
        para_file = f"{run_dir}/params.json"
        with open(para_file, "r") as fpara:
            para_data = json.load(fpara)
        for case_file in files:
            ...
            # Relation Specificity Score
            key = "relation_specific_prompts_probs"
            sum_key_discrete = "relation_specific_success"
            sum_key_cont = "relation_specific_diff"
            if key in data["post"]:
                cur_sum[sum_key_discrete].append(
                    np.mean(
                        [
        ...
        for prefix in ["pre", "post"]:
            for k_efficacy, k_generalization, k_specificity in [
                ...
                if k_generalization in cur_sum and k_specificity in cur_sum and "relation_specific_success" in cur_sum:
                    cur_sum[f"{prefix}_score"] = (
                        hmean(
                            [
                                ...
                                cur_sum["relation_specific_success"][0],
                            ]
                        ),
                        np.nan,
                    )
