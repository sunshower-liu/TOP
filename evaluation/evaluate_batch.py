...
from .py.eval_utils_zsre import compute_rewrite_quality_zsre
...
DS_DICT = {
    "mcf": (MultiCounterFactDataset, compute_rewrite_quality_counterfact_rs),
...
def main(
    ...
    for record_chunks in chunks(ds, num_edits):
        ...
        for i in range(len(record_chunks)):
            rs_topk_idx, rs_mask, rs_best = rs_probability_topk(model, tok, record_chunks[i]["relation_specific_prompts"]) # l * 10
            record_chunks[i]["rs_topk"] = rs_topk_idx
            record_chunks[i]["rs_mask"] = rs_mask
            record_chunks[i]["rs_best"] = rs_best
        record_chunks_copy = deepcopy(record_chunks)
        ...
        record_chunks = record_chunks_copy
...
if __name__ == "__main__":
    ...
    main(
        ...
        dir_name=f"batch_{args.alg_name}_{args.model_name}",
